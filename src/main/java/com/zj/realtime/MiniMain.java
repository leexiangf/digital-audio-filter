...
public class MiniMain {
    ...
    public static void main(String[] args) {
        //String filename = "data/16k-0.pcm";
        String filename = "data/16k-001.mp3";
        if (args.length >= 1) {
            filename = args[0];
        }
        // 日志格式和语言
    ...
    private MiniMain(String filename) {
        ...
        try {
           // inputStream = new FileInputStream(file);
            ...
            AudioFormat mp3Format = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED, 16,
                    audioFormat.getSampleSizeInBits(), audioFormat.getChannels(),
                    audioFormat.getChannels() * 2, audioFormat.getSampleRate(), false);
            inputStream = AudioSystem.getAudioInputStream(mp3Format, audioInputStream);
        ...
        private void sendAudioFrames(WebSocket webSocket) {
            ...
            do {
                ...
                if (readSize > 0) { // readSize = -1 代表流结束
                    ...
                    //logger.info(bytesToSend.toString());
